...
> **Tier C-disjoint.** Let S = {chr7, chr9p, chr10} — the canonical GBM copy-number chromosomes (chr7 gain, chr10 loss, CDKN2A deletion at 9p21). Clone identity **G** and clone abundance change **Ab(clone)** are computed from inferCNV signal restricted to S. Stage B's feature matrix X then **excludes every gene located on any chromosome in S**.
